      ...
      <div class="problem-list">
        <div v-for="(problem, index) in filteredproblems" :key="problem.id" class="problem-item">
          <div v-if="updateIndex !== index" class="problem-content" @click="showProblemDetail(problem, $event)">
            ...
            <p><span class="problem-text"><strong>문제</strong>: {{ truncateText(problem.question) }} </span></p>
            <p><strong>정답:</strong> <span class="problem-text">{{ truncateText(problem.solution) }} </span></p>
...
.main-page {
  ...
  background: linear-gradient(45deg,rgba(255,244,255,0.05) 40%, rgba(255,0,164,0.09));
  /** background: white; **/
  color: #191f28;
  min-height: 100vh;
  display: flex;
